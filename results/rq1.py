...
for file_name in file_list:
    ...
    for learning_type in result_dict.keys():
        ...
        mac_f1 = (f1pbr+f1tbr)/2
        case_name = "_".join(learning_type.split("_")[:-1])
        case_results[case_name][learning_type.split("_")[-1]] = mac_f1    
        if case_name not in case_results_detail.keys():
            case_results_detail[case_name] = {}
        case_results_detail[case_name][learning_type.split("_")[-1]] = [f1pbr, f1tbr]
        # bug_nums[case_name][learning_type.split("_")[-1]] = all_bug_num
    
    for case_name in case_results.keys():
        sorted_results = sorted(case_results[case_name].items(), key=lambda x:x[1], reverse=True)
        for result in sorted_results:
            fold_num = result[0]
            average_result_dict[case_name] = case_results_detail[case_name][result[0]]


model_names = ["MNB","SVM","DNN","CNN","MTCNN","distilbert-base-uncased","distilroberta-base"]
text_features = ["SHORT","LONG","ALL"]
training_types = ["BR","SF","PROTEC"]
...
for model_name in model_names:
    for text_feature in text_features:
        ...
        for training_type in training_types:                   
            f1pbr = average_result_dict[key+"_"+training_type][0]
            f1tbr = average_result_dict[key+"_"+training_type][1]
            macf1 = (f1pbr+f1tbr) / 2
            print(f1pbr, f1tbr, macf1, end=" ")
        print()
