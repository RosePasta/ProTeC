    ...
    for project in projects:
        ...
        for i in range(0, 10):
            ...
            for t_i in range(len(y_bug_test)):
                ...
                if y_pred == 1:
                    pred_type = "TB"
                writer.write(model_name+"_MT_"+training_set+"_"+str(iterator)+"\t"+str(i)+"\t"+identifier+"\t"+real_type+"\t"+pred_type+"\n")
                writer.close()

            f1_pbr, f1_tbr = own_util.evaluation(correct_pb, all_pb_num, correct_tb, all_tb_num)
            print(model_name+"_MT_"+training_set+"_"+str(iterator)+"\t"+str(i), dataset, project, correct_pb, '/', all_pb_num, correct_tb, '/', all_tb_num, round(f1_pbr,3), round(f1_tbr,3))
