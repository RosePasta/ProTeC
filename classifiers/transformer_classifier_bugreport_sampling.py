...
training_set_type = "BR"
which_field = "LONG" # "ALL", "SHORT", "LONG"
...
for dataset in datasets:
    ...
    for project in projects:
        for i in range(0, 10):
            ...
            # Resampling
            ros_input_ids, train_label = ros.fit_resample(train_encodings["input_ids"], train_labels)            
            train_df = {"input_ids":[], "attention_mask":[]}
            for data in ros_input_ids:
                find_attention_mask = False
                for input_id, attention_mask in zip(train_encodings["input_ids"], train_encodings["attention_mask"]):
                    # print(input_id, data)
                    if input_id == data:           
                        train_df["input_ids"].append(data)             
                        train_df["attention_mask"].append(attention_mask)
                        find_attention_mask = True
                        break
            
            train_df = pd.DataFrame.from_dict(train_df)            
            
            train_dataset = ProTeCDataset(train_df, train_labels)
            val_dataset = ProTeCDataset(val_encodings, val_labels)    
                                
            from transformers import AutoModelForSequenceClassification, TrainingArguments, Trainer
            model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=2)
            training_args = TrainingArguments(
                output_dir="./results_"+executed_file_name,
            ...
            for t_i, test_instance in enumerate(test_texts):
                ...
                if predicted_class_id == 1:
                    pred_type = "TB"
                writer.write(model_name+"_"+which_field+"_BR_"+str(iterator)+"\t"+str(i)+"\t"+identifier+"\t"+real_type+"\t"+pred_type+"\n")
                writer.close()
            ...
            print(model_name+"_"+which_field+"_BR_"+str(iterator)+"\t"+str(i), dataset, project, )#÷correct_pb, '/', all_pb_num, correct_tb, '/', all_tb_num, round(f1_pbr,3), round(f1_tbr,3))
