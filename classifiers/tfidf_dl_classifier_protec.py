...
for dataset in datasets:
    ...
    for project in projects:
        ...
        for i in range(0, 10):
            ...
            # 5. Tuning the model
            trained_model.compile(loss='binary_crossentropy',optimizer=optimizers.Adam(lr*0.1),metrics=['acc'])
            callbacks = [EarlyStopping(monitor='val_loss',patience=patience),
                    ModelCheckpoint(filepath="./model/dump_"+executed_file_name, monitor='val_loss',save_best_only=True)]
        
            trained_model.fit(X_bug_train, y_bug_train, epochs=epoch, verbose = 0, callbacks=callbacks,  validation_data = (XValidation,YValidation) )                
            trained_model = keras.models.load_model("./model/dump_"+executed_file_name)
